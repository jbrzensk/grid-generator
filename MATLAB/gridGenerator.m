...
title(sprintf('C-shaped boundary: C*=%.3f, BFGS-optimized bubbles (N_x=%d, N_y=%d)', C_opt, Kx, Ky));
xlabel('x'); ylabel('y');
...
% ==================== Subfunction: sharp-cornered C boundary ===============
function [ X, Y ] = makeGridHardC( nptsx, nptsy )
% Sharp-cornered C boundary in MATLAB (returns (nptsy+1) x (nptsx+1) grids)
...
% % (Optional) visualize boundaries
% figure('Color','w'); hold on
% plot(xb_bdry, yb_bdry, 'b-', 'LineWidth',1.5) % bottom
% plot(xt_bdry, yt_bdry, 'r-', 'LineWidth',1.5) % top
% plot(xl_bdry, yl_bdry, 'g-', 'LineWidth',1.5) % left
% plot(xr_bdry, yr_bdry, 'm-', 'LineWidth',1.5) % right
% axis equal; grid on; title('Sharp-cornered C boundary')

%% Interpolate interior with transfinite interpolation
X = zeros(n+1, m+1);
Y = zeros(n+1, m+1);
...
% % (Optional) plot the full grid
% figure('Color','w'); hold on
% for jj = 1:n+1, plot(X(jj,:), Y(jj,:), 'k-'); end
% for ii = 1:m+1, plot(X(:,ii), Y(:,ii), 'k-'); end
% axis equal; grid on; title('Structured C-shaped Grid (Interior + Boundaries)')
end
